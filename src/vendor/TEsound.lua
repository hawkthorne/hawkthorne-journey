...
function TEsound.playSfx( sound, overlay )
    if string.find( sound , 'audio/' ) ~= 1 then -- not a path
        sound = 'audio/sfx/' .. sound .. '.ogg'
    end
    
    local _sound = TEsound.getSource( sound )

    if overlay and not _sound:isStopped() then
        -- make a secondary source and play it
        _orig = sound
        sound = sound .. '2'
        if not TEsound.source_cache[sound] then
            print( 'secondary' )
            TEsound.source_cache[sound] = TEsound.newsource( _orig, 'static'  )
        end
    else

    ...
    update = function( dt, enemy, player, level )
        ...
        elseif enemy.state == 'default' and player.position.y <= enemy.position.y + 100 then
            if player.position.x < enemy.position.x then
                ...
                if player.position.x + player.width + 25 >= enemy.position.x and player.character.direction == 'left' then
                    enemy.state = 'dive'
                    enemy.fly_dir = -1
                ...
                if player.position.x - 25 <= enemy.position.x + enemy.width and player.character.direction == 'right' then
                    enemy.state = 'dive'
                    enemy.fly_dir = 1
                    enemy.launch_y = enemy.position.y
                end
